...
  if (!project) return null;
  ...
  // 상세 설명 또는 기본 설명 중 하나를 선택합니다.
  const sourceHtml = project.detailedDescription || project.description;

  // 마크다운과 줄바꿈을 HTML로 변환합니다.
  const descriptionHtml = sourceHtml
    ? sourceHtml
        .replace(/\n/g, '<br/>') // 줄바꿈(\n)을 HTML <br>로
        .replace(/####\s*(.*)/g, '<h3>$1</h3>') // #### (헤더)를 <h3>로
        .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') // **굵은 글씨**를 <strong>로
    : '<p>상세 설명이 없습니다.</p>';

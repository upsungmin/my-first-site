...
// 1. SocialLinksMap 컴포넌트 (Hero() 함수 바로 위에 추가)
const SocialLinksMap = ({ links, showSocialEditor, setShowSocialEditor, saveData, showIconPicker, setShowIconPicker, setLinks, isEditMode }) => {
  // 이 컴포넌트의 실제 로직은 복잡하지만, 오류 해결을 위해 단순화합니다.
  ...
  const renderSocialIcon = (link, index) => {
    ...
    return (
      <a
        ...
        className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black transition-all hover:scale-110"
  ...
  // 편집 모드가 아니면 아이콘 목록만 렌더링
  if (!isEditMode) {
    return (
      <div className="flex space-x-4">
        {links.map(renderSocialIcon)}
      </div>
    )
  }

  // 편집 모드 로직 (간단하게 '설정' 버튼만 표시)
  return (
    <div className="flex items-center space-x-4">
        ...
        {/* 실제 에디터 모달은 여기 있어야 하지만, 일단 생략 */}
        {showSocialEditor && (
            <div className="text-sm p-2 bg-yellow-100 text-black rounded-lg">
                소셜 링크 편집기 활성화됨 (실제 UI는 구현 필요)
            </div>
        )}
    </div>
  )
